...
class CPUPlayer(Player):
    ...
    def bot_turn(self, current_table, pot, current_stake):
        ...
        if len(current_table) > 0:
            if len(self.hand) != 2:
                print(
                    f"Warning: {self.name} has {len(self.hand)} hand cards, expected 2"
                )
                self.is_folded = True
                return {"action": "fold"}

            if len(current_table) > 5:
                print(f"Warning: Table has {len(current_table)} cards, expected max 5")
                self.is_folded = True
                return {"action": "fold"}
            ...
            try:
                ...
                print(f"Error evaluating hand for {self.name}: {e}")
                print(
                    f"Hand cards: {len(self.hand)}, Table cards: {len(current_table)}"
                )
                self.is_folded = True
                return {"action": "fold"}
